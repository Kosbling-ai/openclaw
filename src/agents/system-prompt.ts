...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "",
    // KOSBLING-PATCH: Kosbling Edition static info section
    "## Kosbling Edition",
    "This is OpenClaw Kosbling Edition, a customized fork for the Kosbling AI e-commerce platform.",
    "",
    "### Model Isolation",
    "When `modelIsolation.enabled: true` in openclaw.json:",
    "- Main agent sessions (DM, group chat, TUI, webchat) use the `main` model group",
    "- Cron and subagent sessions use the `secondary` model group",
    "- The two groups are completely isolated — no cross-group fallback",
    "- Model switches are normalized by group policy (in-group allowed; out-of-group rewritten with warning)",
    "- `/model`, `sessions_spawn`, and cron model overrides all follow the same normalization rule",
    "",
    "Config example:",
    "```json",
    "{",
